...
import { safelyDeliverSafeArea, safelyInitializePreview, safelyPostMessage } from "./manus-runtime-safety";
export { safelyDeliverSafeArea, safelyInitializePreview, safelyInvoke, safelyPostMessage } from "./manus-runtime-safety";
...
function sendToParent(type: MessageType, payload: Record<string, unknown> = {}): void {
  ...
  if (safelyPostMessage(() => window.parent.postMessage(message, "*"))) {
...
function handleMessage(event: MessageEvent<unknown>): void {
  ...
  if (payload.type === "setSafeAreaInsets" && isValidInsets(payload.payload) && safeAreaCallback) {
    ...
    safelyDeliverSafeArea(() => safeAreaCallback?.({ insets, frame }));
    log(
